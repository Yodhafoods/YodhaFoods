...
const searches = [
    { label: "turmeric powder", icon: Sprout, href: "/search?q=spices", highlight: false },
    { label: "tomato soup", icon: Soup, href: "/search?q=soup", highlight: false },
    { label: "fruit powders..", icon: Apple, href: "/search?q=fruit+powders", highlight: false },
    { label: "ginger powder", icon: Apple, href: "/search?q=ginger+powder", highlight: false },
    { label: "moringa powder", icon: Apple, href: "/search?q=moringa+powder", highlight: false },
    ...
    return (
        <section className="py-6 px-6">
            <div className="md:max-w-4xl mx-auto text-center">
                ...
                <div className="w-full flex flex-wrap justify-center gap-1.5 md:gap-4">
                    {searches.map((item, index) => (
                        <Link
                            key={index}
                            href={item.href}
                            className={`
                                group flex items-center justify-center gap-1 md:gap-2 px-2.5 py-1.5 md:px-6 md:py-3 rounded-full text-xs md:text-base font-medium transition-all duration-300
                                    ${item.highlight
